...
public class GameBoard {
   ...
   * Generate a new Game Board that is a matrix of Cells with the params row and col count.
...
=======
  // procedure read() do
  /**
   * Read game board from stdin
   * In a for cicle reads the next token from the stdin and generates a new Cells object, then keeps
   * that Cell in the atribure gameBoard at the position row and col index.
  ...
  /**
   * Valdiates game board
   * Cehck te game board and calls validate function from each Cell.
   * @return boolean
   */
  public boolean validate() {
    // for 0 to gameBoard(row) do
...
>>>>>>> 0d7bae5 (linting corrections)
  ...
  // procedure searchBoardFigures() do
  /**
   * Search Board FIgures
   * Check the game board matrix searching fugures and if find one append it to the
   * boardFigures atribute list.
   */
  public void searchFigures() {
    // for rowIndex to gameBoard(row) do
    for (int rowIndex = 0; rowIndex < this.gameBoard.length; rowIndex++) {
  ...
  private void searchLFigure(int rowIndex, int colIndex) {
    ...
    if (curentCell.horizontalFigure != null
      ...
      if (curentCell.verticalFigure != null
        ...
        if (curentCell.verticalFigure.items() >= 3
            && curentCell.horizontalFigure.items() >= 3) {
          if (curentCell.LFigure == null) {
            // LFigure := new Figure('L')
            // LFigure.figure.add(horizontalFigure and verticalFigure cells)
            Figure LFigure = Figure.combine(curentCell.horizontalFigure,
                curentCell.verticalFigure, 'L');
            this.boardFigures.add(LFigure);
      ...
      if ((!curentCell.verticalFigure.isLastOrFirstCell(curentCell)
        ...
        if (curentCell.verticalFigure.items() >= 3
          ...
          if (curentCell.TFigure == null) {
            // LFigure := new Figure('T')
            // LFigure.figure.add(horizontalFigure and verticalFigure cells)
            Figure TFigure = Figure.combine(curentCell.horizontalFigure,
                curentCell.verticalFigure, 'T');
            this.boardFigures.add(TFigure);
          }
        }
      }
